...
@scope_check
def all_routes(text):
    config_data = storage_option().config_load_headers(
        request.headers.get("x-api-key"))
    if(request.method.lower() != "options"):
        target_url = config_data['x-target-aircache']
        try:
            redis_key = request.method.lower() + "_" + request.headers.get("x-api-key") + \
                "_" + text + "_"+cache_option().key_params(request)
                    ...
                    if(config_data["x-api-cache"] == "true"):
                        if("x-api-cache-options" in config_data):
                            ...
                            for method_item in method_options:
                                if(method_item.replace(" ", "").lower() == request.method.lower()):
                                    is_cachable = True
                            if(is_cachable):
                                cache_option().create_key(redis_key+"_content",
                                                          result.content.decode('utf-8'), expiry_val)
                            ...
                            cache_option().create_key(redis_key+"_method", result.status_code, expiry_val)
                response = Response(
                    result.content, result.status_code, headers)
            return response
        except Exception:
...
def make_request(target_url, config_data):
    not_allowed_headers = ['content-encoding', 'content-length', 'transfer-encoding',
                           'connection', 'access-control-allow-origin', "user-agent", "host"]
    object_dict = dict(zip(config_data["head_key"], config_data["head_value"]))
    copy = {key: value for (
        key, value) in request.headers if key.lower() not in not_allowed_headers}
